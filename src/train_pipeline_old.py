...
class MLPipeline:
    ...
    def __init__(self, symbol="BTCUSDT", interval="30s"):
        ...
        self.preprocessor = RobustScaler()  # Direct sklearn scaler
        self.ensemble = EnsembleModel(n_classes=2)  # Binary: 0=Down, 1=Up (always directional)
    ...
    def preprocess_data(self, df_features, undersample=True, sequence_length=30, selected_features=None):
        ...
        # Manual data preparation with 80/20 split
        ...
        print(f"Validation: 0 samples (0.0%)")
        ...
        print(f"✓ Fitted robust scaler on {len(X_train)} training samples\n")
        
        # Prepare LSTM sequences
        print(f"Preparing LSTM sequences...")
        X_train_lstm, y_train_lstm = self._create_sequences(X_train_scaled, y_train, sequence_length)
        X_test_lstm, y_test_lstm = self._create_sequences(X_test_scaled, y_test, sequence_length)
        
        print(f"✓ Created {len(X_train_lstm)} sequences of length {sequence_length}")
        print(f"  Shape: {X_train_lstm.shape} (samples, timesteps, features)")
        print(f"✓ Created {len(X_test_lstm)} sequences of length {sequence_length}")
        print(f"  Shape: {X_test_lstm.shape} (samples, timesteps, features)")
    ...
    def _print_metrics(self, metrics, dataset_name="Dataset", y_true=None, y_pred=None, dataset_size=None):
        ...
        # Per-class metrics
        print(f"\n📈 PER-CLASS METRICS:")
        class_names = ['Down ⬇️', 'Up ⬆️']  # Binary directional classification
        ...
        if y_true is not None and y_pred is not None:
            ...
            all_class_names = ['Down ⬇️', 'Up ⬆️']  # Binary directional classification
    ...
    def save_models(self, save_dir='data/models', metrics=None, threshold_percent=0.2, lookahead_periods=6):
        ...
        metadata = {
            ...
            'n_classes': 2,  # Binary classification: 0=Down, 1=Up
            'training_date': timestamp,
            'class_weights': self.prepared_data['class_weights'],
            'model_architecture': 'Stacked Ensemble: (CatBoost + Random Forest + Logistic) → Meta-Learner (Binary Direction)',
    ...
    def run_kfold_validation(self, n_splits=5):
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
            ...
            # Split data (X and y are numpy arrays)
...
def main():
    ...
    LOOKAHEAD = 15  # Look 15 periods ahead (15 minutes for 1m intervals) - increased for better signal/noise ratio
    RUN_CV = True  # Enable k-fold cross-validation
    
    print("\n" + "="*60)
    print("BINARY DIRECTIONAL MODEL - TRAINING:")
