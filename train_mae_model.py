"""
MAE Transformer Training Pipeline
Trains the winning MAE model with increased data for production deployment
"""
...
DAYS = 365  # 1 YEAR OF DATA for robust training
SEQUENCE_LENGTH = 60
...
class MAEPipeline:
    ...
    def train_model(self, X_train, y_train, X_test, y_test):
        ...
        print("\n" + "=" * 80)
        print("STEP 3: MODEL TRAINING (MAE LOSS)")
        print("=" * 80)
    ...
    def save_model(self, results):
        ...
        model_dir = f"data/models/{self.symbol}_{self.interval}_mae_{self.timestamp}"
        os.makedirs(model_dir, exist_ok=True)
        
        # Save model using Keras save method
        model_path = os.path.join(model_dir, 'mae_model.keras')
        self.model.model.save(model_path)  # Use the underlying Keras model
...
def main():
    ...
    print("\n")
    print("╔" + "═" * 78 + "╗")
    print("║" + " " * 20 + "MAE TRANSFORMER TRAINING PIPELINE" + " " * 25 + "║")
    print("║" + " " * 78 + "║")
    print("║" + f"  Symbol: {SYMBOL:<15} Interval: {INTERVAL:<15} Lookahead: {LOOKAHEAD} periods" + " " * 15 + "║")
    print("║" + f"  Training Data: {DAYS} days" + " " * 53 + "║")
    print("╚" + "═" * 78 + "╝")
    print("\n")
    
    pipeline = MAEPipeline()
    ...
    print("\n" + "=" * 80)
    print("Ready for deployment! See DEPLOYMENT_GUIDE.md")
    print("=" * 80)
